...
class Paypal_lib {
	...
	function __construct()
	{
		...
		$gateway_settings = $this->CI->Payment_gateway_settings_model->get_by(array('gateway_id' => $payment_gateway['paypal']));
		...
		if(isset($currency_code))
			$paypal_lib_currency_code = $currency_code;
	...
	function paypal_auto_form() 
	{
		...
		*/
		...
		$paypal_html = '<h2>Processing Payment...</h2>';
		//$paypal_html .= '<body style="text-align:center;" onLoad="document.forms[\'paypal_auto_form\'].submit();">';
		$paypal_html .= '<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>';
		$paypal_html .= $this->paypal_form('paypal_auto_form');
	...
	{
		$str = '';
		$default_currency_code = $this->CI->config->item('paypal_lib_currency_code');
		...
		foreach ($this->fields as $name => $value) {
			...
			if($name == 'amount' and $default_currency_code != $current_currency_code) {
				// convert amount from current currency to default
				$value = $this->currency_converter($current_currency_code, $default_currency_code, $value);
			}
