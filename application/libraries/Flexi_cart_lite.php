...
class Flexi_cart_lite
{
	public function __construct()
	{
		...
		$site_models = config_item('site_models');
		
		$this->CI->load->model($site_models['ecommerce'].'/flexi_cart_lite_model');
	}
	...
	public function sub_total($inc_discount = TRUE, $format = TRUE, $internal_value = FALSE)
	{
		if ($inc_discount)
		{
			...
			$sub_total = ($cart_taxable_value + $cart_non_taxable_value) - $this->CI->flexi->cart_contents['summary']['shipping_total'];
		}
		else
		{
			// The non-discounted cart summary total is to be used, therefore the total already includes the surcharge value.
			$surcharge_total = 0;
			// modified by vinoth_cart.php
			$sub_total = ($this->CI->flexi->cart_contents['summary']['total'] - $this->CI->flexi->cart_contents['summary']['tax_total'] - $this->CI->flexi->cart_contents['summary']['shipping_total']);
		}
	...
	public function get_formated_currency($internal_format = FALSE, $currency_name = FALSE)
	{
		return $this->CI->flexi_cart_lite_model->get_currency_symbol($internal_format, $currency_name);
	}
	
	public function get_currency_code($internal_format = FALSE, $currency_name = FALSE)
	{
		return $this->CI->flexi_cart_lite_model->get_currency_code($internal_format = FALSE, $currency_name = FALSE);
	}
